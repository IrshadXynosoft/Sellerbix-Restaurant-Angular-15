...
import { BatchList } from '../batch-production-list/batch-production-list.component';
...
@Component({
  ...
  styleUrls: ['./stock-issue-report.component.scss']
})
export class StockIssueReportComponent implements OnInit {
  ...
  public displayedColumns: string[] = ['index','process_no','process_date','process_type','name', 'staff','location', 'button'];
 ...
 public generatePurchaseOrderReport!: UntypedFormGroup;
  ...
  viewDetailIssue(id:any)
  {
    this.getIssuesArray.forEach((element:any) => {
      if(id==element.id)
      {
        this.dataservice.setData('stockIssueItem',element);
        this.router.navigate(['inventory/stockissuedetail'])
