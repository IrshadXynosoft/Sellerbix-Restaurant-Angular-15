...
@Component({
  selector: 'app-edit-menu-request',
  templateUrl: './edit-menu-request.component.html',
  styleUrls: ['./edit-menu-request.component.scss']
})
export class EditMenuRequestComponent implements OnInit {
  ...
  getRequestArray = [];
  branch_name: any
  req_id:any=this.route.snapshot.params.id;
  menuRequest:any;
  ...
  getRequestArrayDetails(){
 
    this.httpService
    .get('menu-request/' + this.req_id)
    .subscribe((result) => {
      if (result.status == 200) {
        this.menuRequest= result.data;
        this.newStockRequestForm.patchValue({
  
          stock_request_number:this.menuRequest.menu_request_number,
          branch_selected: this.menuRequest.from_branch_name,
          from_branch:this.menuRequest.from_branch_id,
          branch_id_for_requst: this.menuRequest.from_branch_id,
          req_date:moment(this.menuRequest.created_at).format('DD/MM/YYYY'),
          comments: this.menuRequest.comments,
          to_branch: this.menuRequest.to_branch_id,
      })
      this.total_price=this.menuRequest.total_amount;
      this.menuRequest.menu_request_item.forEach((element:any) => {
        let objData = {
          stock_type:4,
         id: element.recipe_id,
          name: element.item_name,
          stock_on_hand: element.available_stock,
          supplier_name: '--',
          ...
          unit_equals_sub_unit:'--',
          unit_equals_measurement_unit:'--',
          qty:parseFloat(element.requested_qty).toFixed(2),
          qtybuyingUnit: '',
          qtybuyingSubUnit: '',
          qtyToMeasurementUnit: element.requested_qty,
          qtyMeasurementUnit: '',
          total: element.total,
          measurement_unit_name:element.measurement_unit_name
        }
        ...
        this.selectedStockIDs.push(objData.id);
      });
  ...
  clearSupplier(index: any) {
    let items = this.newStockRequestForm.get('suppliersData') as UntypedFormArray;
    let form_data = this.newStockRequestForm.value;
    this.selectedStockIDs.splice(this.selectedStockIDs.indexOf(form_data.suppliersData[index].id), 1);
  ...
  saveStockRequest() {
    ...
    if (arrayLength == item.length && item.length > 0) {
      let postParamsSave = {
        ...
        request_items_list: recipe
        
      }

      this.httpService.put('menu-request/'+this.menuRequest.id, postParamsSave)
        .subscribe(result => {
          if (result.status == 200) {
