...
@Component({
  selector: 'app-menu-transfer',
  templateUrl: './menu-transfer.component.html',
  styleUrls: ['./menu-transfer.component.scss']
})
export class MenuTransferComponent implements OnInit {
  ...
  public displayedColumnsForReceived: string[] = ['index', 'menu_request_number', 'date', 'item_count', 'staff', 'location', 'button'];
  public displayedColumnsForProcessed: string[] = ['index', 'menu_request_number', 'date', 'item_count', 'staff', 'location','button'];
  ...
  getStockRequests() {
    this.dataSourceForReceived.data =[];
    this.httpService
      .get('list-all-menu-stock-received-request/' + this.branch_id)
      .subscribe((result) => {
  ...
  getProcessedRequests() {
    ...
    this.httpService
      .get('list-all-menu-stock-transfered/' + this.branch_id)
      .subscribe((result) => {
        if (result.status == 200) {
  ...
  receiveRequest(id: any) {
    this.getRequestArray.forEach((element: any) => {
      if (id == element.id) {
        this.dataService.setData('menureceivetItem', element);
        this.router.navigate(['inventory/receivemenustockrequest'])
  ...
  deleteRequestDetail(id: any, reqNumber: any) {
    ...
    this.dialogService.confirmed().subscribe(confirmed => {
      if (confirmed) {
        ...
        this.httpService.put('menu-request/' + id,postParams)
          .subscribe(result => {
            if (result.status == 200) {
              this.snackBService.openSnackBar("Menu Request- " + reqNumber + " Rejected Successfully!!", "Close");
              this.getStockRequests();
            } else {
    ...
    this.dialogService.confirmed().subscribe(confirmed => {
      if (confirmed) {
        ...
        this.httpService.put('menu-request/' + id,postParams)
          .subscribe(result => {
            if (result.status == 200) {
              this.snackBService.openSnackBar("Menu Request- " + reqNumber + " Accepted Successfully!!", "Close");
              this.getStockRequests();
            } else {
              this.snackBService.openSnackBar(result.message, "Close");
              console.log("Error");
