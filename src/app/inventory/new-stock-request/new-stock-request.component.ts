...
@Component({
  selector: 'app-new-stock-request',
  templateUrl: './new-stock-request.component.html',
  styleUrls: ['./new-stock-request.component.scss']
})
export class NewStockRequestComponent implements OnInit {
        ...
        if (result.status == 200) {
          ...
          this.finishedGoodArray?.forEach((element: any) => {
            let objData = {
              ...
              unit_equals_measurement_unit: element.purchase_detail ? element.purchase_detail.unit_equals_measurement_unit : '',
              qty: "",
              qtybuyingUnit: '',
              qtybuyingSubUnit: '',
              qtyToMeasurementUnit: '',
              qtyMeasurementUnit: '',
              total: 0,
              measurement_unit_name: element.purchase_detail ? element.purchase_detail.measurement_unit_name : ''
            }
            this.options.push(objData)
          });
  ...
  unit(index: any) {
    ...
    if(form_data.suppliersData[index].stock_type==3){
      value='1 ' +form_data.suppliersData[index].measurement_unit_name
    }
    else{
  ...
  measurementunit(index: any) {
    let form_data = this.newStockRequestForm.value;
    return form_data.suppliersData[index].buying_unit?form_data.suppliersData[index].buying_unit:form_data.suppliersData[index].measurement_unit_name;
  }
