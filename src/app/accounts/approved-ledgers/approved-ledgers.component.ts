...
@Component({
  selector: 'app-approved-ledgers',
  templateUrl: './approved-ledgers.component.html',
  styleUrls: ['./approved-ledgers.component.scss']
})
export class ApprovedLedgersComponent implements OnInit, AfterViewInit {
  ...
  getExpenseCategory() {
    this.httpService.get('payments/1', false)
      .subscribe(result => {
        if (result.status == 200) {
          // let approvedcategory=[];
          // approvedcategory =result.data.categories;
          // approvedcategory.forEach((obj:any) => {
          //   if(obj.status == 1){
