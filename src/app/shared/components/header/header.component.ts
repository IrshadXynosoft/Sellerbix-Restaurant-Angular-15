...
export class HeaderComponent implements OnInit {
  ...
  constructor(private readonly newOrderMqtt: NewOrderService, public dialog: MatDialog, private dialogService: ConfirmationDialogService, private location: Location, private router: Router, private httpService: HttpServiceService, private snackBService: SnackBarService, private localService: LocalStorage, private dataService: DataService, private printMqtt: PrintMqttService
  ) {
  ...
  subscribeToTopic() {
    ...
    this.subscription = this.newOrderMqtt.topic(this.messageType)
      .subscribe((data: IMqttMessage) => {
        ...
        if (result.status == 0) {
          ...
          if (this.notification_status == 'true') {
            this.audio.load();
            this.audio.play();
            this.snackBService.openSnackBar("Order no:  #" + result.order_no + ' ' + message + "  by driver", "")
          }
        }
      });
    // **
    //  code for old listener
    this.oldSubscription = this.newOrderMqtt.topic(this.oldMessage)
      .subscribe((data: IMqttMessage) => {
        let item = data.payload.toString();
  ...
  logout() {
    ...
    this.dialogService.confirmed().subscribe(confirmed => {
      if (confirmed) {
        this.httpService.post('logout', null)
          .subscribe(result => {
  ...
  openDrawer() {
    this.httpService.get('open-drawer')
      .subscribe(result => {
        if (result.status == 200) {
          this.printMqtt.publish('print', JSON.stringify(result.data[0].printObj))
            .subscribe((data: any) => {
            });
        }
        else {
  ...
  busyMode() {
    const title = this.busyModeCheck == false || this.busyModeCheck == 0 ? 'Deactivate' : 'Activate'
    const options = {
    ...
    this.dialogService.confirmed().subscribe(confirmed => {
      if (confirmed) {
        ...
        this.httpService.post('change-status-busy-mode', body)
          .subscribe(result => {
            if (result.status == 200) {
              this.snackBService.openSnackBar(result.message, "Close");
