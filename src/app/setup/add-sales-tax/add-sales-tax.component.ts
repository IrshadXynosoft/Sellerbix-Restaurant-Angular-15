...
export class AddSalesTaxComponent implements OnInit {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<AddSalesTaxComponent>,
  ...
  onBuildForm() {
    this.taxForm = this.formBuilder.group({
      ...
      rate: [
        '',
        [Validators.required, Validators.pattern(this.validationFloat)],
      ],
      is_default: false,
  ...
  addTax() {
    ...
    if (
      this.Subs.value.length > 0 &&
      parseFloat(total) != parseFloat(this.taxForm.value['rate'])
    ) {
      this.snackBService.openSnackBar(
        'Tax rate should be equal to sum of split tax rates',
        'Close'
      );
    } else {
      let post = {
      ...
      this.httpService.post('tax', post).subscribe((result) => {
