...
@Component({
  selector: 'app-edit-section-heads',
  templateUrl: './edit-section-heads.component.html',
  styleUrls: ['./edit-section-heads.component.scss']
})
export class EditSectionHeadsComponent implements OnInit {
...
filteredOptions: Observable<any[]> | undefined;
  constructor(public dialog: MatDialog, public dialogRef: MatDialogRef<EditSectionHeadsComponent>, public formBuilder: UntypedFormBuilder, private httpService: HttpServiceService, private snackBService: SnackBarService,private localService:LocalStorage,@Inject(MAT_DIALOG_DATA) public data: {branch_id:string,id:string}) {
    }

  ngOnInit(): void {
    this.onBuildForm();
    this.getItems();
    this.getSectionHeads();
  ...
  getSectionHeads(){
    this.httpService.get('online/sections/'+this.data.id,false)
    .subscribe(result => {
      if (result.status == 200) {
        this.sectionHeadData=result.data
        this.sectionForm.patchValue({
         name:this.sectionHeadData.name,
         secondary_name:this.sectionHeadData.secondary_name
        })
        if(this.sectionHeadData.menu_section_mapping){
          this.sectionHeadData.menu_section_mapping.forEach((element:any) => {
            if(element.item){
              this.postRecords.push(element.item_id);
        ...
        console.log("Error in SectionHeads");
      }
    });
  }
  ...
  editSectionHead() {
      ...
      this.httpService.put('online/sections/'+this.data.id, post)
        .subscribe(result => {
          if (result.status == 200) {
            this.snackBService.openSnackBar("Section Head updated successfully.", "Close");
            this.close();
          } else {
            if(result.data){
