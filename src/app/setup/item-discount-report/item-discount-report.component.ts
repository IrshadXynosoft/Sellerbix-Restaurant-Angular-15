...
@Component({
  selector: 'app-item-discount-report',
  templateUrl: './item-discount-report.component.html',
  styleUrls: ['./item-discount-report.component.scss']
})
export class ItemDiscountReportComponent implements OnInit {
  @ViewChild('reportTable', { read: MatPaginator, static: false })
  set pagination(value: MatPaginator) {
    this.dataSourceitemDiscount.paginator = value;
  }
  ...
  public dataSourceitemDiscount = new MatTableDataSource<itemReport>();
  ...
  ngAfterViewInit(): void {
    this.dataSourceitemDiscount.sort = this.sort;
  }
  ...
  generateReport() {
    ...
    this.dataSourceitemDiscount.data = [];
    if (postParams) {
      this.httpService
        .post('item-discount-report', postParams)
        .subscribe((result) => {
          if (result.status == 200) {
            this.itemDiscounts = result.data;
            this.dataSourceitemDiscount.data = [];
            const item_data: any = [];

            this.itemDiscounts.forEach((element: any) => {
              if (element.amount != 0) {
                let objData = {
                  name: element.name,
                ...
                item_data.push(objData);
              }
            });

            this.dataSourceitemDiscount.data = item_data as itemReport[];
          } else {
            this.snackBService.openSnackBar(result.message, 'Close');
          }
        });
