...
import { UntypedFormArray, UntypedFormBuilder, UntypedFormGroup, Validators, UntypedFormControl, FormControl } from '@angular/forms';
...
export class EditMenuItemComponent implements OnInit {
  ...
  constructor(private constant: Constants, private route: ActivatedRoute, private snackBService: SnackBarService, private httpService: HttpServiceService, private router: Router, public dialog: MatDialog, private formBuilder: UntypedFormBuilder) {
  }
  ...
  ngOnInit() {
    ...
    this.modifierlist_filteredOptions = this.list_autocomplete.valueChanges.pipe(
      startWith(''),
  ...
  private _filterlist(value: string): string[] {
    const filterValue = value.toLowerCase();
    return this.list_options.filter((option: any) => option.name.toLowerCase().includes(filterValue));
  }

  onBuildForm() {
    this.addmenuForm = this.formBuilder.group({
      item_name: ['', Validators.compose([Validators.required, Validators.maxLength(75)])],
      secondary_name: [''],
      barcode: [''],
      item_code: ['', Validators.compose([Validators.required])],
      default_price: ['', Validators.compose([Validators.required, Validators.pattern(this.numericExpression)])],
      description: [''],
  ...
  onBuildModifierListForm() {
    this.modifierListForm = this.formBuilder.group({
      ...
      item_price: ['', Validators.compose([Validators.required, Validators.pattern(this.numericExpression)])],
    })
  }
  ...
  addmodifierlist(groupIndex: any) {
    if (this.modifierListForm.valid && this.list_autocomplete.value) {
      ...
      if (this.AddedModifierGroupRecords[groupIndex].modifier_list.length > 0) {
        this.AddedModifierGroupRecords[groupIndex].modifier_list.forEach((obj: any) => {
          if (obj.item_name.toLowerCase() == this.list_autocomplete.value.toLowerCase()) {
            flag = true;
          }
        });
      }
      if (!flag) {
        ...
        this.AddedModifierGroupRecords[groupIndex].modifier_list.push(modifier_list);
  ...
  updateListValue(groupindex: any, listindex: any, value: any) {
    this.AddedModifierGroupRecords[groupindex].modifier_list[listindex].item_price = value;
  }


  deletemodifierlist(groupIndex: any, listIndex: any) {
    this.AddedModifierGroupRecords[groupIndex].modifier_list.splice(listIndex, 1);
    this.validModifierCheck();
  }
  ...
  getProducts() {
    this.httpService.get('branch-item')
      .subscribe(result => {
        if (result.status == 200) {
  ...
  getMenuEditSingle() {
    this.httpService.get('item/' + this.route.snapshot.params.id)
      .subscribe(result => {
        if (result.status == 200) {
          ...
          if (result.data[0].group_item) {
            ...
            groupitemlist.forEach((obj: any) => {
              this.GroupItems.push({ item_id: obj.item_id, items_list_status: obj.items_list_status })
            });
          }
          this.getBranch();
          ...
          this.getMenuItemsCategory(this.singleItemRecords[0].category.category_id)
          this.specificationArray[0].value = this.singleItemRecords[0].item_specifications ? this.singleItemRecords[0].item_specifications.calories : null;
          this.specificationArray[1].value = this.singleItemRecords[0].item_specifications ? this.singleItemRecords[0].item_specifications.carb : null;
          this.specificationArray[2].value = this.singleItemRecords[0].item_specifications ? this.singleItemRecords[0].item_specifications.fat : null;
          this.specificationArray[3].value = this.singleItemRecords[0].item_specifications ? this.singleItemRecords[0].item_specifications.protien : null;
          this.specificationArray[4].value = this.singleItemRecords[0].item_specifications ? this.singleItemRecords[0].item_specifications.preparation_time : null;
          ...
          this.tags = this.singleItemRecords[0].tags ? this.singleItemRecords[0].tags : []
        }
        else {
  ...
  onChange(event: any, item: any) {
    ...
    if (item.food_symbol_id && item.is_selected) {
      ...
      this.foodsymbolSelected.splice(this.foodsymbolSelected.indexOf(item.id), 1)
    }
  }
  ...
  getMenuItemsCategory(id: any) {
    this.httpService.get('category-item' + '/' + id)
      .subscribe(result => {
        if (result.status == 200) {
  ...
  getMenuItemsCategoryFromEvent(event: any) {
    this.httpService.get('category-item' + '/' + event.target.value)
      .subscribe(result => {
        if (result.status == 200) {
          this.categoryItemRecords = result.data.category_items;
          this.categoryItemRecords.forEach((obj: any) => {
  ...
  getFoodSymbols() {
    this.httpService.get('food-symbol')
      .subscribe(result => {
        if (result.status == 200) {
    ...
    this.httpService.get('branch', false).subscribe(result => {
      if (result.status == 200) {
        ...
        this.branchrecords.forEach((obj: any) => {
          ...
          let priceData: any;
          this.locationPrices = [];
          this.singleItemRecords[0].item_location_price?.forEach((obj2: any) => {
            priceData = {
        ...
        this.entitieslist.forEach((dataObj: any) => {
          let items = this.addGroupedmenuForm.get('entities') as UntypedFormArray;
        ...
        this.locationPrices.forEach((dataObj: any) => {
          let items = this.addmenuForm.get('location_prices') as UntypedFormArray;
          items.push(this.createPriceItem(dataObj));
        });

        this.locationPrices.forEach((dataObj: any) => {
          let items = this.addCombomenuForm.get('location_prices') as UntypedFormArray;
    ...
    if (event.checked) {
      if (this.GroupItems.length > 0) {
        ...
        if (!this.GroupItems.find(object => object['item_id'] == item.id)) {
          this.GroupItems.push({ item_id: item.id, items_list_status: item.items_list_status });
        }
        // });
      }
      else {
  ...
  showSlide(index: any, item: any) { // for enabling toggle
    let found = this.GroupItems.filter((obj: { item_id: any; }) => obj.item_id == item.id)
    if (found.length > 0) {
      return false;
    }
    return true
  ...
  RegularEntityValidationCheck() {
    ...
    this.addmenuForm.value['entities'].forEach((obj: any, i: any) => {
      if (obj.call_center || obj.dine_in || obj.e_order || obj.pos || obj.take_away || obj.walk_in) {
        flag[i] = true;
      }
    });
  ...
  UpdateMenuItem() {
    ...
    if (this.addmenuForm.valid && this.ValidModifierList && this.foodsymbolSelected.length > 0 && !this.branchPriceValidityCheck && this.entitiesValidityCheck && this.modifierListZeroEntryCheck) {
      const body = {
        ...
        "location_prices": !this.addmenuForm.value['allow_all_location'] ? this.addmenuForm.value['location_prices'] : [],
        ...
        "specifications": {
          calories: this.specificationArray[0].value ? this.specificationArray[0].value : 0.00,
          carbs: this.specificationArray[1].value ? this.specificationArray[1].value : 0.00,
          fat: this.specificationArray[2].value ? this.specificationArray[2].value : 0.00,
          protien: this.specificationArray[3].value ? this.specificationArray[3].value : 0.00,
          preparation_time: this.specificationArray[4].value ? this.specificationArray[4].value : 0.00
        },
        "schedules": this.schedule.value ? this.schedule.value : []
      }
      this.httpService.put('item/' + this.route.snapshot.params.id, body)
        .subscribe(result => {
          if (result.status == 200) {
            this.snackBService.openSnackBar("Regular Menu Item Updated", "Close");
  ...
  ComboBranchPriceValidityCheck() {
    this.branchPriceValidityCheck = true;
    this.addCombomenuForm.controls['location_prices'].value.forEach((obj: any) => {
      if (obj.location_price == "" || this.invalidFlag) {
        this.branchPriceValidityCheck = true;
      }
      else {
  ...
  updateComboMenuItem() {
    ...
    if (this.addCombomenuForm.valid && !this.affectedPriceValidityCheck && !this.branchPriceValidityCheck && this.entitiesValidityCheck) {
      const body = {
        ...
        "food_symbols": this.foodsymbolSelected,
        "location_prices": !this.addCombomenuForm.value['allow_all_location'] ? this.addCombomenuForm.value['location_prices'] : [],
  ...
  UpdateGroupMenuItem() {
    ...
    if (this.addGroupedmenuForm.valid && this.entitiesValidityCheck && this.GroupItems.length > 0 && this.foodsymbolSelected.length > 0) {
      const body = {
      ...
      this.httpService.put('item/' + this.route.snapshot.params.id, body)
        .subscribe(result => {
          if (result.status == 200) {
            this.snackBService.openSnackBar("Grouped Menu Item Updated", "Close");
            this.router.navigate(['setup/menuHeader/menu'])
          } else {
            if (result.data) {
  ...
  addGroup(flag: any, groupindex: any = null): void {
    if (flag == 'edit') {
      const dialogRef = this.dialog.open(AddGroupComponent, {
        ...
        data: {
          header: "Edit Group",
          modifier_group_name: this.AddedModifierGroupRecords[groupindex].modifier_group_name,
          limit_minimum: this.AddedModifierGroupRecords[groupindex].limit_minimum,
          limit_maximum: this.AddedModifierGroupRecords[groupindex].limit_maximum,
          modifier_group_id: this.AddedModifierGroupRecords[groupindex].modifier_group_id,
          can_add_multiple: this.AddedModifierGroupRecords[groupindex].can_add_multiple,
          flagset: false
        }
      });
      dialogRef.afterClosed().subscribe(result => {
        if (result.length > 0) {
          result = result[0];
          this.AddedModifierGroupRecords[groupindex].modifier_group_name = result.modifier_group_name
          this.AddedModifierGroupRecords[groupindex].limit_minimum = result.limit_minimum
          this.AddedModifierGroupRecords[groupindex].limit_maximum = result.limit_maximum
          this.AddedModifierGroupRecords[groupindex].can_add_multiple = result.can_add_multiple
          this.AddedModifierGroupRecords[groupindex].modifier_group_id = result.modifier_group_id
          this.validModifierCheck();
        }
      });
      ...
      dialogRef.afterClosed().subscribe(result => {
        if (result.length > 0) {
          ...
          if (this.AddedModifierGroupRecords.length > 0) {
            this.AddedModifierGroupRecords.forEach((obj: any) => {
              if (obj.modifier_group_name.toLowerCase() == result.modifier_group_name.toLowerCase()) {
                flag = true;
              }
            });
          }
