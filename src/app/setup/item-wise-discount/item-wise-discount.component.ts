...
export class ItemWiseDiscountComponent implements OnInit {
  ...
  public displayedColumns: string[] = ['index', 'name', 'branch_price','selected_discount_type', 'discount_value', 'amount_after_discount'];
  ...
  getInnerMenu() {
    this.httpService.post('online/item-discount-setup', {tenant_id:4,branch_id:this.id})
    .subscribe(result => {
      if (result.status == 200) {
        const data: any = [];
            ...
            obj.item.forEach((element: any) => {
             ...
             if(element.item_location_price){
              let items = {
                ...
                amount_after_discount: element.item_location_price.item_discount_price,
                selected_discount_type: element.item_location_price.discount_type,
    ...
    this.innerMenuArray.forEach((element:any) => {
      if(element.name==textLabel)
      {
        ...
        element.menu.forEach((obj:any) => {
          ...
          else if(obj.selected_discount_type=='percentage' && obj.discount_value>0){
            rate=((obj.discount_value* (obj.branch_price)) / 100).toFixed(2);
            itemAmountAfterdiscount = (parseFloat(obj.branch_price) - parseFloat(rate)).toFixed(2);
          }
          else if(obj.selected_discount_type=='value' && obj.discount_value>0){
            itemAmountAfterdiscount = (parseFloat(obj.branch_price)- parseFloat(obj.discount_value)).toFixed(2);
          }
  ...
  discountChanged(discountId: any, element: any) {
    ...
    let rate: any
    let itemAmountAfterdiscount:any = 0.00
    ...
    if(discountId=='none' || element.discount_value==0){
      ...
      itemAmountAfterdiscount = (parseFloat(element.branch_price) - parseFloat(rate)).toFixed(2);
    }
    else if(discountId=='value' && element.discount_value>0){
      itemAmountAfterdiscount = (parseFloat(element.branch_price)- parseFloat(element.discount_value)).toFixed(2);
    }
      ...
      priceChanged(discountAmount:any,element:any){
          ...
          else if(element.selected_discount_type=='percentage' && discountAmount>0){
            rate=((discountAmount* (element.branch_price)) / 100).toFixed(2);
            itemAmountAfterdiscount = (parseFloat(element.branch_price) - parseFloat(rate)).toFixed(2);
          }
          else if(element.selected_discount_type=='value' && discountAmount>0){
            itemAmountAfterdiscount = (parseFloat(element.branch_price)- parseFloat(discountAmount)).toFixed(2);
          }
              this.dataSource.data[index].amount_after_discount=itemAmountAfterdiscount
  ...
  saveBranchMenu(data: any) {
    ...
    if (this.isMenuSave) {
      ...
      this.httpService.post('online/update-item-discount', postParam)
        .subscribe(result => {
          if (result.status == 200) {
            this.getInnerMenu();
